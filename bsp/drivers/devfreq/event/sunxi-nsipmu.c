				  ...
				  struct devfreq_event_data *edata)
{
	...
	sunxi_debug(NULL, "dram_clk:%luM load:%lu rw:%luM total:%luM\n",
				(unsigned long)(clk_get_rate(info->dram_clk) / 1000000),
				(unsigned long)((edata->load_count >> 7) * 100 / (edata->total_count >> 7)),
				(unsigned long)(edata->load_count / 100000), (unsigned long)(edata->total_count / 100000));
