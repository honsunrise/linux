...
static struct clk *di_dclk_gate;
...
#ifdef DI_MULPLEX_SUPPORT
	spin_lock(&gsunxi_dev->slock);
	if (gsunxi_dev->cur_di) {
		gsunxi_dev->cur_di->busy = false;
		gsunxi_dev->cur_di = NULL;
		gsunxi_dev->busy = false;
	}
	spin_unlock(&gsunxi_dev->slock);
#endif
	di_irq_clear();
	di_reset();
	return IRQ_HANDLED;
...
static s32 di_clk_cfg(struct device_node *node)
{
	...
	di_dclk_gate = of_clk_get(node, 3);
	if (!di_dclk_gate || IS_ERR(di_dclk_gate)) {
		pr_err("try to get di_dclk_gate clock failed!\n");
		return -1;
	}
...
static void di_clk_uncfg(void)
{
	...
	if (di_dclk_gate == NULL || IS_ERR(di_dclk_gate)) {
		pr_err("di_dclk_gate handle is invalid, just return!\n");
		return;
	} else {
		clk_disable_unprepare(di_dclk_gate);
		clk_put(di_dclk_gate);
		di_dclk_gate = NULL;
	}
...
static s32 di_clk_enable(void)
{
	...
	if (di_dclk_gate == NULL || IS_ERR(di_dclk_gate)) {
		...
		if (clk_prepare_enable(di_dclk_gate)) {
			pr_err("try to enable di_dclk_gate failed!\n");
			return -1;
		}
	}
