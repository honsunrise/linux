...
static const char *goodix_ts_name = "goodix-secondary-ts";
static const char *goodix_input_phys = "input/ts_secondary";
...
#ifdef CONFIG_TOUCHSCREEN_GT9XX_SECONDARY_UPDATE
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	gtp_config_proc = proc_mkdir("gt9xx_secondary_fw", NULL);
	if (!gtp_config_proc)
		return -ENODEV;

	/*  Create proc file system */
	entry = proc_create(GT91XX_SECONDARY_CONFIG_PROC_FILE, 0664,
				      gtp_config_proc, &config_proc_ops);
	if (!entry)
		dev_err(&client->dev, "create_proc_entry %s failed\n",
			GT91XX_SECONDARY_CONFIG_PROC_FILE);
	else
...
exit_free_config_proc:
	remove_proc_entry(GT91XX_SECONDARY_CONFIG_PROC_FILE, gtp_config_proc);
	return -ENODEV;
}
...
#ifdef CONFIG_TOUCHSCREEN_GT9XX_SECONDARY_TOOL
...
static void gtp_drv_remove(struct i2c_client *client)
{
	...
	remove_proc_entry(GT91XX_SECONDARY_CONFIG_PROC_FILE, gtp_config_proc);
	remove_proc_entry("gt9xx_secondary_fw", NULL);
...
{
	pr_info("Gt9xx_secondary driver exited\n");
...
MODULE_AUTHOR("wanglixin <wanglixin@allwinnertech.com>");
