...
/*
 * Allwinner SUNXI MBUS driver for SUN8IW21
 *
...
#define MBUS_SW_CLK_ON_REG(n)       (0x0030 + (0x4 * (n)))

/* Software Clock OFF Register 0: 1=dis-access */
#define MBUS_SW_CLK_OFF_REG(n)      (0x0040 + (0x4 * (n)))
 ...
 *
 * Note: If you change @enum mbus_port{} member, change
 *     @struct port_index{} as well.
 */
enum mbus_port {
	MBUS_PORT_CPU		= 0,
	MBUS_PORT_MAHB		= 2,
...
/*
 * @struct port_index{}:
 * Used to iterate over all masters when get value.
 * See mbus_get_value() for details!
 */
struct port_index {
	u32 cpu;
	u32 mahb;
	...
	MBUS_PORT_CPU,   MBUS_PORT_MAHB,  MBUS_PORT_DMA,
	MBUS_PORT_VE,    MBUS_PORT_CSI,   MBUS_PORT_ISP,
	MBUS_PORT_G2D,   MBUS_PORT_DE,    MBUS_PORT_IOMMU,
	MBUS_PORT_NPU,   MBUS_PORT_RISCV, MBUS_PORT_CE,
	MBUS_PORTS_MAX,
};
...
#define MBUS_SENSOR_DEVICE_ATTR_RO(name, index) \
		SENSOR_DEVICE_ATTR(name, 0400, mbus_show_value, NULL, index)
#define MBUS_SENSOR_DEVICE_ATTR_RW(name, index) \
		SENSOR_DEVICE_ATTR(name, 0644, mbus_show_value, mbus_store_value, index)
#define SENSOR_DEVICE_ATTR_PTR(name) &sensor_dev_attr_##name.dev_attr.attr

static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_cpuddr,     MBUS_PMU_CPU);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_riscv_sys,  MBUS_PMU_RISCV_SYS);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_mahb_ddr,   MBUS_PMU_MAHB);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_dma_ddr,    MBUS_PMU_DMA);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_ve_ddr,     MBUS_PMU_VE);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_ve1_ddr,    MBUS_PMU_VE1);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_ce_ddr,     MBUS_PMU_CE);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_csi_ddr,    MBUS_PMU_CSI);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_isp_sys,    MBUS_PMU_ISP);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_g2d_ddr,    MBUS_PMU_G2D);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_de_ddr,     MBUS_PMU_DE);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_iommu_ddr,  MBUS_PMU_IOMMU);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_npu_ddr,    MBUS_PMU_NPU);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_othddr,     MBUS_PMU_OTH);
static MBUS_SENSOR_DEVICE_ATTR_RO(pmu_totddr,     MBUS_PMU_TOTAL);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_prio,      MBUS_PORT_PRI);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_qos,       MBUS_PORT_QOS);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_watt,      MBUS_PORT_WT);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_acs,       MBUS_PORT_ACS);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_bwl0,      MBUS_PORT_BWL0);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_bwl1,      MBUS_PORT_BWL1);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_bwl2,      MBUS_PORT_BWL2);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_bwlen,     MBUS_PORT_BWLEN);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_abs_bwlen, MBUS_PORT_ABS_BWLEN);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_abs_bwl,   MBUS_PORT_ABS_BWL);
static MBUS_SENSOR_DEVICE_ATTR_RW(port_bw_satu,   MBUS_PORT_BW_SATU);
...
MODULE_DESCRIPTION("SUNXI SUN8IW21 MBUS Driver");
