...
#if IS_ENABLED(CONFIG_AW_MACADDR_MGT)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)
	if (get_custom_mac_address(1, "wifi", macAddr) > 0) {
		memcpy(hw_priv->addresses[0].addr, macAddr, ETH_ALEN);
		atbm_set_p2p_vif_addr(hw_priv);
		goto out;
	}
#else
	if (get_wifi_custom_mac_address(readmac) > 0) {
		sscanf(readmac, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
...
int access_file(char *path, char *buffer, int size, int isRead)
{
	...
	if (isRead)
	{
...
#endif
		{
			...
			ret = __vfs_read(fp,buffer,size,&fp->f_pos);
#else
			ret = fp->f_op->read(fp, buffer, size, &fp->f_pos);
#endif
#ifdef set_fs
			set_fs(old_fs);
#endif
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0))
			ret = __vfs_write(fp, buffer,size, &fp->f_pos);
#else
			ret = fp->f_op->write(fp, buffer, size, &fp->f_pos);
#endif
#ifdef set_fs
			set_fs(old_fs);
#endif
