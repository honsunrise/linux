...
static void ssv6006_adj_config(struct ssv_hw *sh)
{
    ...
    if ((sh->cfg.chip_identity == SV6167Q) || (sh->cfg.chip_identity == SV6267Q)){
        sh->cfg.external_pa |= EX_PA_ENABLE;
    } else {
        if (sh->cfg.external_pa & EX_PA_ENABLE){
            sh->cfg.external_pa = 0;
...
#define SV6256P_IC      0x6256
    ...
    sh->cfg.disable_dpd = ((sh->flash_config.padpd) ? 0x0 : 0x1f); // bit 0 2,4g, bit 1 ~ 4 5g band padpd
