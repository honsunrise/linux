...
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)))
...
static void _import_default_cfg (char *stacfgpath)
{
    ...
    buf=(char *)vmalloc(MAX_CHARS_PER_LINE);
    if(NULL==buf)
    {
        return;
    }

    cfg_value=(char *)vmalloc(MAX_CHARS_PER_LINE);
    if(NULL==cfg_value)
    {
        vfree((void *)buf);
        return;
	...
	do {
		...
		memset(cfg_value, '\0', MAX_CHARS_PER_LINE);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
