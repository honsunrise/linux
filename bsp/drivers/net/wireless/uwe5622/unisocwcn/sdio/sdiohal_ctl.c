...
struct timespec64 tp_tx_start_time;
struct timespec64 tp_tx_stop_time;
int tp_tx_cnt;
int tp_tx_flag;
...
static void sdiohal_throughput_tx_compute_time(void)
{
	...
	if (tp_tx_cnt % 500 == 0) {
		...
		tp_tx_stop_time.tv_nsec = now.tv_nsec;
		...
		tp_tx_start_time.tv_nsec = now.tv_nsec;
	}
		   ...
		   struct mbuf_t *tail, int num)
{
	...
	if (tp_rx_cnt / (500*64) == 1) {
		...
		tp_rx_stop_time.tv_nsec = now.tv_nsec;
		...
		tp_rx_start_time.tv_nsec = now.tv_nsec;
	}
	tp_tm_begin = ktime_to_timespec64(ktime_get_boottime());
...
static char *sdiohal_firmware_data(unsigned long int imag_size)
{
	...
	char *buffer = NULL;
	char *filename = (char *)FIRMWARE_PATH;
