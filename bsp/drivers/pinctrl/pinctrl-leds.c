...
struct sunxi_dirgpio {
	...
	char **gpio_names;
	int supply_num;
	struct regulator *gpio_regulator[MAX_SUPPLY_NUM];
};
...
static int sunxi_dirgpio_resource_get(struct sunxi_dirgpio *chip)
{
	...
	err = of_property_read_u32(chip->node, "supply-num", &chip->supply_num);
	if (err || !chip->supply_num || chip->supply_num > MAX_SUPPLY_NUM) {
		chip->supply_num = 0;
		dev_dbg(chip->dev, "No need supply power\n");
	} else {
		for (i = 0; i < chip->supply_num; i++) {
			sprintf(supply_name, "gpio%d", i + 1);
			chip->gpio_regulator[i] = devm_regulator_get_optional(&(chip->pdev->dev), supply_name);
		}
	}
...
static int sunxi_dirgpio_enable_power(struct sunxi_dirgpio *chip)
{
	int ret = 0, i;

	for (i = 0; i < chip->supply_num; i++) {
		ret = regulator_enable(chip->gpio_regulator[i]);
		if (ret) {
			dev_err(chip->dev, "fail to enable gpio%d-regulator\n", i+1);
			return ret;
		}
	}
...
static int sunxi_dirgpio_disable_power(struct sunxi_dirgpio *chip)
{
	int ret = 0, i = 0;

	for (i = 0; i < chip->supply_num; i++) {
		ret = regulator_disable(chip->gpio_regulator[i]);
		if (ret) {
			dev_err(chip->dev, "fail to disable gpio%d-regulator\n", i+1);
			return ret;
		}
	}
...
static int sunxi_dirgpio_hw_init(struct sunxi_dirgpio *chip)
{
	...
	for (i = 0; i < chip->gpio_num; i++) {
		...
		/* if gpio_sdcard_reused == 1, reuse pin port of sdcard, it no need to request gpio */
		if (!chip->gpio_sdcard_reused) {
			err = devm_gpio_request(chip->dev, gpio, dev_name(chip->dev));
			if (err) {
				dev_err(chip->dev, "gpio_pin_%d(%d) request failed\n", i + 1, gpio);
				return err;
			}
		}
		dev_dbg(chip->dev, "gpio_pin_%d(%d), init flag is %d, valid\n", i + 1, gpio, flag);
		...
		chip->pins[i].name = devm_kasprintf(chip->dev,
				GFP_KERNEL, "%s", chip->gpio_names[i]);
		if (!chip->pins[i].name)
			return -ENOMEM;
...
static int sunxi_dirgpio_probe(struct platform_device *pdev)
{
	...
	/* enable gpio power before setup gpio */
	err = sunxi_dirgpio_enable_power(chip);
	if (err) {
...
static void sunxi_dirgpio_remove(struct platform_device *pdev)
{
	...
	sunxi_dirgpio_disable_power(chip);
...
static int sunxi_dirgpio_resume(struct device *dev)
{
	...
	sunxi_dirgpio_enable_power(chip);
	return 0;
}
...
static const struct of_device_id sunxi_dirgpio_of_match[] = {
	{.compatible = "allwinner,sunxi-direct-gpio", .data = NULL},
	{.compatible = "allwinner,sunxi-gpio-leds", .data = NULL},
	{ /* sentinel */ }
};
