...
#define VER_REG_OFFS 0x00000024
	...
	*version = 0;
	if (ver == 0xff) {
		...
		if (io == NULL) {
			sunxi_err(NULL, "%s: ioremap of sys_cfg register failed!\n",
				__func__);
			return -1;
		}
...
static int gpu_domain_probe(struct platform_device *pdev)
{
	...
	if (!np) {
		sunxi_err(&pdev->dev, "failed to match gpu power domain\n");
		return -ENODEV;
	}

	pd = devm_kzalloc(&pdev->dev, sizeof(*pd), GFP_KERNEL);
	if (!pd) {
		sunxi_err(&pdev->dev, "failed to malloc pd space\n");
		return -ENOMEM;
	}
	...
	if (IS_ERR(pd->regmap)) {
		sunxi_err(&pdev->dev, "no regmap available\n");
	...
	if (pd->irq == -EPROBE_DEFER) {
		sunxi_err(&pdev->dev, "failed to find gpu domain IRQ number\n");
		return -EPROBE_DEFER;
	}
	...
	if (ret) {
		sunxi_err(&pdev->dev, "failed to request gpu domain interrupt\n");
		return ret;
	}
	...
	if (IS_ERR(pd->clk)) {
		sunxi_err(&pdev->dev, "failed to find clock\n");
		return -ENOMEM;
	}
