...
static irqreturn_t sunxi_scr_interrupt(int irqno, void *dev_id)
{
	...
	if (irq_status & SCR_INTSTA_RXPERR) {
		SCR_ERR("SmartCard Rx Parity Error!!\n");
	}

	if (irq_status & SCR_INTSTA_ATRFAIL) {
		SCR_DBG("SmartCard ATR Fail!!, again set ACT!\n");
		if (pscr->atr_resp == SCR_ATR_RESP_AGAIN)
			pscr->atr_resp = SCR_ATR_RESP_FAIL;
		else
			pscr->atr_resp = SCR_ATR_RESP_AGAIN;
...
static void scr_timer_handler(struct timer_list *list)
{
...
static long sunxi_scr_ioctl(struct file *file, uint32_t cmd, unsigned long arg)
{
	...
	case SCR_IOCGATR:
		while (SCR_ATR_RESP_OK != pscr->atr_resp && try_num--) {
			msleep(10);
			if ((pscr->rx_transmit_status == SCR_RX_TRANSMIT_TMOUT)
				&& (pscr->atr_resp == SCR_ATR_RESP_AGAIN)
				&& (scr_buf_rx.cnt > 0)) {
				memcpy(pscr->scr_atr_des.atr_data,
					scr_buf_rx.buf, scr_buf_rx.cnt);
				pscr->scr_atr_des.atr_len = scr_buf_rx.cnt;
				pscr->atr_resp = SCR_ATR_RESP_OK;
				/* parse ATR data to reconfig smart card */
	...
	case SCR_IOCWRDATA: {
		...
		if (5 == scr_buf_tx.cnt) {
			...
			wr_data.rtn_len = scr_buf_tx.buf[4];
			wr_data.psw1 = scr_buf_rx.buf[rtn_data_len - 2];
			wr_data.psw2 = scr_buf_rx.buf[rtn_data_len - 1];
			ret = copy_to_user((void __user *)arg,
				&wr_data,
				sizeof(struct scr_wr_data)) ? -EFAULT : 0;
			if (ret != 0)
				return ret;
		 ...
		 */
			...
			wr_data.psw1 = scr_buf_rx.buf[1];
			wr_data.psw2 = scr_buf_rx.buf[2];
			ret = copy_to_user((void __user *)arg,
				&wr_data,
				sizeof(struct scr_wr_data)) ? -EFAULT : 0;
		...
		/* type3: CLS + INS + P1 + P2 + lc + data +le -> le+lc  */
			...
			wr_data.rtn_len = scr_buf_tx.buf[scr_buf_tx.cnt-1];
			wr_data.psw1 = scr_buf_rx.buf[rtn_data_len - 2];
			wr_data.psw2 = scr_buf_rx.buf[rtn_data_len - 1];
			ret = copy_to_user((void __user *)arg,
				&wr_data,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
	timer_setup(&pscr->poll_timer, scr_timer_handler, 0);
#else
