 ...
 *
 * Copyright (c) 2024, wuhao <wuhao@allwinnertech.com>
 *
...
static SOC_ENUM_SINGLE_DECL(sunxi_dac_enum, SUNXI_DAC_AN_REG, LINEOUT_DIFFEN, sunxi_differ_text);
...
static int snd_sunxi_clk_init(struct platform_device *pdev, struct sunxi_codec_clk *clk)
{
	...
	/* get bus clk */
	clk->clk_bus = of_clk_get_by_name(np, "clk_bus_audio0_apb");
	...
	/* get module clk */
	clk->clk_adda_dac = of_clk_get_by_name(np, "clk_audiocodec0_dac");
	if (IS_ERR_OR_NULL(clk->clk_adda_dac)) {
		SND_LOG_ERR("clk_adda_dac get failed\n");
...
static int snd_sunxi_clk_enable(struct sunxi_codec_clk *clk)
{
	...
	if (clk_prepare_enable(clk->clk_pll_peri1_600m)) {
		SND_LOG_ERR_STD(E_AUDIOCODEC_SWDEP_CLK_EN, "clk_pll_peri1_600m enable failed\n");
		ret = -EINVAL;
		goto err_enable_clk_pll_peri1_600m;
	}

	if (clk_prepare_enable(clk->clk_adda_dac)) {
		SND_LOG_ERR("clk_adda_dac enable failed\n");
		ret = -EINVAL;
