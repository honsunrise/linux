				   ...
				   struct tvd_buffer *buffer)
{
	...
	if (buffer == NULL || buffer->dma_address == 0) {
		tvd_wrn("%s: vb_buf->priv is NULL!\n", __func__);
		return;
	}
	addr_org = buffer->dma_address;
...
int tvd_open_special(int video_id)
{
	struct tvd_dev *dev = tvd[video_id];
	int ret = -1;
	int i = 0;
...
int tvd_close_special(int video_id)
{
	struct tvd_dev *dev = tvd[video_id];
	int ret = 0;
	int i = 0;
...
int vidioc_s_fmt_vid_cap_special(int video_id, struct v4l2_format *f)
{
	struct tvd_dev *dev = tvd[video_id];
	int ret = 0;
...
int vidioc_g_fmt_vid_cap_special(int video_id, struct v4l2_format *f)
{
	struct tvd_dev *dev = tvd[video_id];
	u32 locked = 0, system = 2;
...
int dqbuffer_special(int video_id, struct tvd_buffer **buf)
{
	int ret = 0;
	unsigned long flags = 0;
	struct tvd_dev *dev = tvd[video_id];
...
int qbuffer_special(int video_id, struct tvd_buffer *buf)
{
	struct tvd_dev *dev = tvd[video_id];
...
int tvd_force_reset_buffer(int video_id)
{
	...
	while (!list_empty(&dma_q->active)) {
		...
		list_del(&buf->list);
		list_add(&buf->list, &donelist->active);
	}
...
EXPORT_SYMBOL(tvd_force_reset_buffer);

int vidioc_streamon_special(int video_id)
{
	struct tvd_dev *dev = tvd[video_id];
	struct tvd_dmaqueue *dma_q = &dev->vidq_special;
...
int vidioc_streamoff_special(int video_id)
{
	struct tvd_dev *dev = tvd[video_id];
	struct tvd_dmaqueue *dma_q = &dev->vidq_special;
	struct tvd_dmaqueue *donelist = &dev->done_special;
...
static void (*tvd_buffer_done)(int video_id);
...
static irqreturn_t tvd_isr_special(int irq, void *priv)
{
	...
	if (tvd_buffer_done)
		...
		pr_err("video source callback is missing!\n");
		return IRQ_HANDLED;
	}
