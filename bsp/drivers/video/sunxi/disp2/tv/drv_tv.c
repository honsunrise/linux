...
#if IS_ENABLED(CONFIG_ARCH_SUN8IW7)
...
({\
	...
	if (sel >= TVE_DEVICE_NUM) {\
		sunxi_warn(NULL, "%s, sel(%d) is out of range\n", __func__, sel);\
		tmp = -1;\
	}		 \
	(tmp);\
})\

#define TV_INFO(fmt, arg...)  sunxi_info(NULL, "[TVE]:"fmt, ##arg)
#define TV_DBG(fmt, arg...)   sunxi_debug(NULL, "%s()%d - "fmt, __func__, __LINE__, ##arg)
#define TV_WRN(fmt, arg...)   sunxi_warn(NULL, "%s()%d - "fmt, __func__, __LINE__, ##arg)
#define TV_ERR(fmt, arg...)   sunxi_err(NULL, "%s()%d - "fmt, __func__, __LINE__, ##arg)
#define TV_DEV_ERR(dev, fmt, arg...)   sunxi_err(dev, "%s()%d - "fmt, __func__, __LINE__, ##arg)
				...
				const char *buf, size_t count)
{
	...
	if (val) {
		tv_fake_detect = 1;
		TV_INFO("fake enable tv_out\n");
	} else {
		tv_fake_detect = 0;
		TV_INFO("fake tv_out disable \n");
	}
	return count;
}
...
void tv_report_hpd_work(u32 sel, u32 hpd)
{
	TV_WRN("there is null report hpd work,you need support the extcon class!");
}

s32 tv_detect_thread(void *parg)
{
	TV_WRN("there is null tv_detect_thread,you need support the extcon class!");
	return -1;
}

s32 tv_detect_enable(u32 sel)
{
	TV_WRN("there is null tv_detect_enable,you need support the extcon class!");
	return -1;
}

s32 tv_detect_disable(u32 sel)
{
	TV_WRN("there is null tv_detect_disable,you need support the extcon class!");
		return -1;
}
static ssize_t tv_faketv_show(struct device *dev,
				struct device_attribute *attr, char *buf)
...
static s32 __get_offset(struct device_node *node, int i)
{
	...
	if (ret < 0) {
		TV_WRN("there is no tve dac(%d) offset value.\n", i);
	} else {
	...
	extcon_dev[sel]->name = &extcon_name[sel][0];
		...
		for (i = 0; i < DAC_COUNT; i++) {
			...
			ret = of_property_read_u32(pdev->dev.of_node, sub_key,
						   &value);
			if (ret < 0) {
				TV_WRN("tve%d have no dac %d, sub_key:%s\n",
					sel, i, sub_key);
			} else {
				dac_no = value;
			...
			ret = of_property_read_u32(pdev->dev.of_node, sub_key,
						   &value);
			if (ret < 0) {
				TV_WRN("tve%d have no type%d sub_key:%s\n",
					sel, i, sub_key);
		...
		TV_DBG("[TV]:value = %d, type = %d, mode = %d\n",
		...
		if ((output_type == DISP_OUTPUT_TYPE_TV)
			...
			TV_DBG("[TV]:g_tv_info.screen[0].tv_mode = %d",
...
static int tv_top_init(struct platform_device *pdev)
{
	...
	if (!g_tv_info.base_addr) {
		TV_DEV_ERR(&pdev->dev, "unable to map tve common registers\n");
		ret = -EINVAL;
		goto err_iomap;
	}
	...
	if (IS_ERR(g_tv_info.bus_clk)) {
		TV_DEV_ERR(&pdev->dev, "fail to get clk for tve common module!\n");
		ret = -EINVAL;
		goto err_iomap;
	}
	...
	if (IS_ERR(g_tv_info.rst_bus)) {
		TV_DEV_ERR(&pdev->dev, "get tv top bus reset control  failed!\n");
		ret = -EINVAL;
		goto err_iomap;
	}
...
static int tv_probe(struct platform_device *pdev)
{
	...
	g_tv_info.screen[pdev->id].rst_bus =
	 ...
	 if (IS_ERR(g_tv_info.screen[pdev->id].rst_bus)) {
		TV_DEV_ERR(&pdev->dev, "get tve bus reset control  failed!\n");
		goto err_iomap;
	}
...
int __init tv_module_init(void)
{
	...
	if (err) {
		TV_ERR("cdev_add fail\n");
		return -1;
	}

	tv_class = class_create(THIS_MODULE, "tv");
	if (IS_ERR(tv_class)) {
		TV_ERR("class_create fail\n");
		return -1;
	}
