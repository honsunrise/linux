...
/*
 * A V4L2 driver for bf30a2 cameras.
 *
...
MODULE_AUTHOR("lcs");
MODULE_DESCRIPTION("A low-level driver for bf30a2_mipi sensors");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0.0");

#define MCLK              (24 * 1000 * 1000)
#define V4L2_IDENT_SENSOR 0xbc
...
/*
 * The bf30a2 i2c address
 */
#define I2C_ADDR 0xdc

#define SENSOR_NAME "bf30a2"
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	if (exp_val > (bf30a2_sensor_vts - 16))
		exp_val = bf30a2_sensor_vts - 16;
	if (exp_val < 16)
		exp_val = 16;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	sensor_read(sd, 0xfc, &rdval);
    sensor_dbg("0xfc 0x%x\n", rdval);
	if (rdval != 0x3b) {
		sensor_err("read 0xfc return value 0x%x\n", rdval);
		//return -ENODEV;
	}

    sensor_read(sd, 0xfd, &rdval);
    sensor_dbg("0xfd 0x%x\n", rdval);
	if (rdval != 0x02) {
		sensor_err("read 0xfd return value 0x%x\n", rdval);
		//return -ENODEV;
	}
