...
unsigned int ov8858version = OV8858B2;
/*
...
static struct regval_list b1_default_regs[] = {
	{0x0103, 0x01},
	{0x303f, 0x01},
	{0x3012, 0x6c},
	{0x0100, 0x00},
	...
	{0x4601, 0xcb},
	{0x470b, 0x00},
	//NO CONTIUSMODE ADD BY PENGTAO FAN 20140723
	//0x4800, 0x6C;
...
static struct regval_list b1_sensor_quxga_regs[] = {
	{0x0100, 0x01},
	{0x0100, 0x00},  //
	{0x030e, 0x00},  //
	{0x0312, 0x01},  //
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	sensor_read(sd, 0x302a, &version);
	if (version == 0xb2) {
		ov8858version = OV8858B2;
		sensor_dbg("ov8858version is 0x%x\n", version);
	} else if (version == 0xb1) {
		ov8858version = OV8858B1;
		sensor_dbg("ov8858version is 0x%x\n", version);
	} else {
		sensor_err("ov8858version is 0x%x, this sensor \n", version);
		return -ENODEV;
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	} else if (ov8858version == OV8858B1) {
		ret = sensor_write_array(sd, b1_default_regs,
					ARRAY_SIZE(b1_default_regs));
		if (ret < 0) {
			sensor_err("write b1_default_regs error\n");
			return ret;
		}
	}
	...
	if (wsize->regs) {
		if (ov8858version == OV8858B2) {
			...
			sensor_write_array(sd, b1_sensor_quxga_regs,
					ARRAY_SIZE(b1_sensor_quxga_regs));
		}
	}
