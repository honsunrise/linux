...
MODULE_AUTHOR("HY");
MODULE_DESCRIPTION("A low-level driver for THINE serdes sensor");
MODULE_LICENSE("GPL");
...
#define SENSOR_NAME "thine_serdes"

struct v4l2_subdev *thine_sd;

// #define __DEBUG_thine
#ifdef __DEBUG_thine

__maybe_unused static unsigned int gpio_i2c_read(struct v4l2_subdev *sd, unsigned short chip_addr, int reg, int bit_flag);
static unsigned int thine_reg_write(struct v4l2_subdev *sd, t_reglist *reg);
static struct class *thine_class;

#define __err(x, arg...) pr_err("[thine]"x, ##arg)
#define __warn(x, arg...) pr_warn("[thine]"x, ##arg)
#define __info(x, arg...) pr_info("[thine]"x, ##arg)
...
static ssize_t thine_debug_store(struct class *class, struct class_attribute *attr,
							const char *buf, size_t count)
{
	...
	thine_debug.data = thine_debug.regaddr & 0x0000ffff;
	thine_debug.reg = (thine_debug.regaddr >> 16) & 0x0000ffff;
	thine_debug.device_addr = (thine_debug.regaddr >> 32) & 0x000000ff;
	thine_debug.wr16_bit = (thine_debug.regaddr >> 40) & 0x01;
	thine_debug.wr_bit = (thine_debug.regaddr >> 44) & 0x01;

	__info("data: 0x%x , reg: 0x%x , wr16_bit: 0x%x , wr_bit: 0x%x !\r\n", thine_debug.data, thine_debug.reg, thine_debug.wr16_bit, thine_debug.wr_bit);
	if (thine_debug.wr_bit) {
		reg.data = thine_debug.data;
		reg.reg_addr = thine_debug.reg;
		reg.device_addr = thine_debug.device_addr;
		reg.wr16 = thine_debug.wr16_bit;
		thine_reg_write(thine_sd, &reg);
		__info("write!\r\n");
	} else {
		__info("read!\r\n");
		if (thine_debug.data > thine_debug.reg) {
			range_count = thine_debug.data - thine_debug.reg;

			for (i = 0; i <= range_count; i++) {
				val = gpio_i2c_read(thine_sd, thine_debug.device_addr, thine_debug.reg + i, thine_debug.wr16_bit);
				__info(" reg[0x%x]: 0x%x ", thine_debug.reg + i, val);
				if (i / 10 > 0) {
					printk("\r\n");
				}
			}
		} else {
			val = gpio_i2c_read(thine_sd, thine_debug.device_addr, thine_debug.reg, thine_debug.wr16_bit);
			__info("chip 0x%x reg 0x%x val = 0x%x\r\n", thine_debug.device_addr, thine_debug.reg, val);
		}
	}
...
static ssize_t thine_debug_show(struct class *class,
					struct class_attribute *attr, char *buf)
{
	...
	__info("chip: 0x%x , data: 0x%x , reg: 0x%x , wr16_bit: 0x%x , wr_bit: 0x%x !\r\n",
		thine_debug.device_addr, thine_debug.data, thine_debug.reg, thine_debug.wr16_bit, thine_debug.wr_bit);

	return count;
}

static struct class_attribute thine_attrs[] = {
	__ATTR(reg, 0755, thine_debug_show, thine_debug_store),
};
#endif
...
__maybe_unused static unsigned int thine_reg_init(struct v4l2_subdev *sd, t_reglist *reg, int reg_size)
{
	int i = 0;

	for (i = 0; i < reg_size; i++) {
			thine_reg_write(sd, &reg[i]);
	}
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	if (wsize->regs)
		thine_reg_init(sd, wsize->regs, wsize->regs_size);
...
#ifdef __DEBUG_thine
	thine_class = class_create(THIS_MODULE, "thine_class");
	if (IS_ERR(thine_class)) {
		__err("%s:%u class_create() failed\n", __func__, __LINE__);
		return PTR_ERR(thine_class);
	}

	/* sys/class/car_reverse/xxx */
	for (i = 0; i < ARRAY_SIZE(thine_attrs); i++) {
		err = class_create_file(thine_class, &thine_attrs[i]);
		if (err) {
			pr_err("%s:%u class_create_file() failed. err=%d\n", __func__, __LINE__, err);
			while (i--) {
				class_remove_file(thine_class, &thine_attrs[i]);
			}
			class_destroy(thine_class);
			thine_class = NULL;
			return err;
		}
	}
