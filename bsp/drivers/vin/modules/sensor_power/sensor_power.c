			 ...
			 int *gpio)
{
	...
	*gpio = gnum;
	sensor_power_dbg("fetch %s gpio = %d\n", name, *gpio);
	return 0;
}
/*
...
static int sensor_gpio_write(struct sensor_power_dev *sensor_power, enum sensor_gpio_type gpio_id, unsigned int out_value)
{
	...
	return sensor_os_gpio_write(gc, out_value);
}
...
static int sensor_power_probe(struct platform_device *pdev)
{
	...
	sensor_power = kzalloc(sizeof(*sensor_power), GFP_KERNEL);
	if (!sensor_power) {
		ret = -ENOMEM;
		goto ekzalloc;
	}
	...
	if (sensor_power->gpio[RESET] >= 0) {
		ret = gpio_request(sensor_power->gpio[RESET], NULL);
		if (ret < 0)
			sensor_power_err("RESET: gpio%d request failed!\n", sensor_power->gpio[RESET]);
	}
	...
	if (sensor_power->gpio[PWDN] >= 0) {
		ret = gpio_request(sensor_power->gpio[PWDN], NULL);
		if (ret < 0)
			sensor_power_err("PWDN: gpio%d request failed!\n", sensor_power->gpio[PWDN]);
	}
	...
	sensor_power_dbg("reset : pin=%d\n", sensor_power->gpio[RESET]);
	sensor_power_dbg("pwdn  : pin=%d \n", sensor_power->gpio[PWDN]);
...
static void sensor_power_remove(struct platform_device *pdev)
{
	...
	if (sensor_power->gpio[RESET] >= 0)
		gpio_free(sensor_power->gpio[RESET]);
	if (sensor_power->gpio[PWDN] >= 0)
...
VIN_INIT_DRIVERS(sensor_power_init);
