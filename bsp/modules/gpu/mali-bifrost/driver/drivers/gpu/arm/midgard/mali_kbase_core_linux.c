...
static int assign_irqs(struct platform_device *pdev)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (!irq_res) {
			dev_err(kbdev->dev, "No clock resource at index %d\n", i);
		}
		irq_res = platform_get_resource(pdev, IORESOURCE_IRQ, i);
		if (!irq_res) {
...
int assign_irqs(struct platform_device *inpdev)
{
	struct kbase_device *kbdev = to_kbase_device(&inpdev->dev);
	...
	for (i = 0; i < ARRAY_SIZE(irq_names_caps); i++) {
		...
		if (irq < 0) {
			dev_err(kbdev->dev, "lbl2 No IRQ resource '%s'\n", irq_names_caps[i]);
			return irq;
		}
