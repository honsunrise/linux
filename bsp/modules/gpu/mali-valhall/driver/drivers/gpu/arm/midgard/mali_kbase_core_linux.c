...
static void sunxi_set_gpu_opp_table_name(struct kbase_device *kbdev)
{
	...
	dev_info(kbdev->dev, "get gpu's volt is %d mv\n", u_volt / 1000);
	/* uV -> mV */
	u_volt = u_volt / 1000;
...
int power_control_init(struct kbase_device *kbdev)
{
	...
	sunxi_set_gpu_opp_table_name(kbdev);
...
void power_control_term(struct kbase_device *kbdev)
{
	 ...
	 * kbdev->opp_table is returned by dev_pm_opp_set_prop_name().
	 *
...
#ifdef CONFIG_OF
	u32 val;
	/* User can disable dvfs by setting dvfs_status = 0 in board.dts */
