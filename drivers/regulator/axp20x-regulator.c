...
static const struct linear_range axp8191_dcdc1_ranges[] = {
	REGULATOR_LINEAR_RANGE(1000000, 0x00, 0x1c, 100000),
};

static const struct linear_range axp8191_dcdc2_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x00, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
};

static const struct linear_range axp8191_dcdc6_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x00, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
...
static const struct linear_range axp8191_dcdc8_ranges[] = {
	REGULATOR_LINEAR_RANGE(500000, 0x00, 0x46, 10000),
	REGULATOR_LINEAR_RANGE(1220000, 0x47, 0x57, 20000),
...
static const struct regulator_desc axp8191_regulators[] = {
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC1, "dcdc1", "vin1",
			      axp8191_dcdc1_ranges, 0x1d,
			      AXP8191_DC1OUT_VOL, GENMASK(4, 0),
			      AXP8191_DCDC_CTL1, BIT(0), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC2, "dcdc2", "vin2",
			      axp8191_dcdc2_ranges, 0x58,
			      AXP8191_DC2OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(1), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC3, "dcdc3", "vin3",
			      axp8191_dcdc2_ranges, 0x58,
			      AXP8191_DC3OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(2), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC4, "dcdc4", "vin4",
			      axp8191_dcdc2_ranges, 0x58,
			      AXP8191_DC4OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(3), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC5, "dcdc5", "vin5",
			      axp8191_dcdc2_ranges, 0x58,
			      AXP8191_DC5OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(4), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC6, "dcdc6", "vin6",
			      axp8191_dcdc6_ranges, 0x71,
			      AXP8191_DC6OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(5), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC7, "dcdc7", "vin7",
			      axp8191_dcdc2_ranges, 0x58,
			      AXP8191_DC7OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(6), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC8, "dcdc8", "vin8",
			      axp8191_dcdc8_ranges, 0x68,
			      AXP8191_DC8OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL1, BIT(7), 250),
	AXP_DESC_RANGES_DELAY(AXP8191, DCDC9, "dcdc9", "vin9",
			      axp8191_dcdc8_ranges, 0x68,
			      AXP8191_DC9OUT_VOL, GENMASK(6, 0),
			      AXP8191_DCDC_CTL2, BIT(0), 250),
	AXP_DESC_SW(AXP8191, DC1SW1, "dc1sw1", "swin",
		    AXP8191_DCDC_CTL2, BIT(3)),
	AXP_DESC_SW(AXP8191, DC1SW2, "dc1sw2", "swin",
		    AXP8191_DCDC_CTL2, BIT(4)),
	AXP_DESC(AXP8191, ALDO1, "aldo1", "aldoin", 500, 3400, 100,
		 AXP8191_ALDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(0)),
	AXP_DESC(AXP8191, ALDO2, "aldo2", "aldoin", 500, 3400, 100,
		 AXP8191_ALDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(1)),
	AXP_DESC(AXP8191, ALDO3, "aldo3", "aldoin", 500, 3400, 100,
		 AXP8191_ALDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(2)),
	AXP_DESC(AXP8191, ALDO4, "aldo4", "aldoin", 500, 3400, 100,
		 AXP8191_ALDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(3)),
	AXP_DESC(AXP8191, ALDO5, "aldo5", "aldoin", 500, 3400, 100,
		 AXP8191_ALDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(4)),
	AXP_DESC(AXP8191, ALDO6, "aldo6", "aldoin", 500, 3400, 100,
		 AXP8191_ALDO6OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(5)),
	AXP_DESC(AXP8191, BLDO1, "bldo1", "bldoin", 500, 3400, 100,
		 AXP8191_BLDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(6)),
	AXP_DESC(AXP8191, BLDO2, "bldo2", "bldoin", 500, 3400, 100,
		 AXP8191_BLDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL1, BIT(7)),
	AXP_DESC(AXP8191, BLDO3, "bldo3", "bldoin", 500, 3400, 100,
		 AXP8191_BLDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(0)),
	AXP_DESC(AXP8191, BLDO4, "bldo4", "bldoin", 500, 3400, 100,
		 AXP8191_BLDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(1)),
	AXP_DESC(AXP8191, BLDO5, "bldo5", "bldoin", 500, 3400, 100,
		 AXP8191_BLDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(2)),
	AXP_DESC(AXP8191, CLDO1, "cldo1", "cldoin", 500, 3400, 100,
		 AXP8191_CLDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(3)),
	AXP_DESC(AXP8191, CLDO2, "cldo2", "cldoin", 500, 3400, 100,
		 AXP8191_CLDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(4)),
	AXP_DESC(AXP8191, CLDO3, "cldo3", "cldoin", 500, 3400, 100,
		 AXP8191_CLDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(5)),
	AXP_DESC(AXP8191, CLDO4, "cldo4", "cldoin", 500, 3400, 100,
		 AXP8191_CLDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(6)),
	AXP_DESC(AXP8191, CLDO5, "cldo5", "cldoin", 500, 3400, 100,
		 AXP8191_CLDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL2, BIT(7)),
	AXP_DESC(AXP8191, DLDO1, "dldo1", "dldoin", 500, 3400, 100,
		 AXP8191_DLDO1OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL3, BIT(0)),
	AXP_DESC(AXP8191, DLDO2, "dldo2", "dldoin", 500, 3400, 100,
		 AXP8191_DLDO2OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL3, BIT(1)),
	AXP_DESC(AXP8191, DLDO3, "dldo3", "dldoin", 500, 3400, 100,
		 AXP8191_DLDO3OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL3, BIT(2)),
	AXP_DESC(AXP8191, DLDO4, "dldo4", "dldoin", 500, 3400, 100,
		 AXP8191_DLDO4OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL3, BIT(3)),
	AXP_DESC(AXP8191, DLDO5, "dldo5", "dldoin", 500, 3400, 100,
		 AXP8191_DLDO5OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL3, BIT(4)),
	AXP_DESC(AXP8191, DLDO6, "dldo6", "dldoin", 500, 3400, 100,
		 AXP8191_DLDO6OUT_VOL, GENMASK(4, 0), AXP8191_LDO_PWR_CTL3, BIT(5)),
	AXP_DESC(AXP8191, ELDO1, "eldo1", "eldoin", 500, 1500, 25,
		 AXP8191_ELDO1OUT_VOL, GENMASK(5, 0), AXP8191_LDO_PWR_CTL3, BIT(6)),
	AXP_DESC(AXP8191, ELDO2, "eldo2", "eldoin", 500, 1500, 25,
		 AXP8191_ELDO2OUT_VOL, GENMASK(5, 0), AXP8191_LDO_PWR_CTL3, BIT(7)),
	AXP_DESC(AXP8191, ELDO3, "eldo3", "eldoin", 500, 1500, 25,
		 AXP8191_ELDO3OUT_VOL, GENMASK(5, 0), AXP8191_LDO_PWR_CTL4, BIT(0)),
	AXP_DESC(AXP8191, ELDO4, "eldo4", "eldoin", 500, 1500, 25,
		 AXP8191_ELDO4OUT_VOL, GENMASK(5, 0), AXP8191_LDO_PWR_CTL4, BIT(1)),
	AXP_DESC(AXP8191, ELDO5, "eldo5", "eldoin", 500, 1500, 25,
		 AXP8191_ELDO5OUT_VOL, GENMASK(5, 0), AXP8191_LDO_PWR_CTL4, BIT(2)),
	AXP_DESC(AXP8191, ELDO6, "eldo6", "eldoin", 500, 1500, 25,
		 AXP8191_ELDO6OUT_VOL, GENMASK(5, 0), AXP8191_LDO_PWR_CTL4, BIT(3)),
	AXP_DESC_FIXED(AXP8191, RTC_LDO, "rtcldo", "ips", 1800),
};
